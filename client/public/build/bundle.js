...
var app = (function () {
    ...
    function create_if_block$1(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(span, file$2, 22, 2, 476);
    		},
    		m: function mount(target, anchor) {
    ...
    function create_fragment$2(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(p, file$2, 11, 0, 278);
    		},
    		l: function claim(nodes) {
    			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
    		},
    ...
    function create_if_block_1(ctx) {
    	...
    	const block = {
    		...
    		p: function update(ctx, dirty) {
    			...
    			if (dirty & /*missingRequirement*/ 64) textfield_changes.error = /*missingRequirement*/ ctx[6];
    			textfield.$set(textfield_changes);
    ...
    // (45:2) {#if user && !authenticating && !serenity}
    function create_if_block(ctx) {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "if",
    		source: "(45:2) {#if user && !authenticating && !serenity}",
    		ctx
    	});
    ...
    // (64:4) {#if !retrievingData && slugs[0].slug != ''}
    function create_if_block_3(ctx) {
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "if",
    		source: "(64:4) {#if !retrievingData && slugs[0].slug != ''}",
    		ctx
    	});
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	let submitSearch = async () => {
    		...
    		if (search.length === 0) return $$invalidate(6, missingRequirement = true);
    		$$invalidate(6, missingRequirement = false);
    		$$invalidate(8, retrievingData = true);
    		$$invalidate(0, [slugs, error] = await getSlugs(search), slugs);
    	...
    	const click_handler_2 = () => submitSearch();
