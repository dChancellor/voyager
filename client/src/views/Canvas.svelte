<script>
  ...
  function setup() {
    ...
    for (i = 0; i <= starTotal; i++) {
      const location = createVector(random(0, width), random(0, height - 400));
  ...
  function draw() {
    ...
    for (i = 5; i > 1; i--) {
      ...
      if (frameCount % 20 == 0) {
        const bob = createVector(0, random(-0.01, 0.01));
        waves[i].bob(bob);
      }
      waves[i].friction();
      waves[i].update();
  ...
  class StarField {
    ...
    strengthChange(i) {
      if (this.starArray[i].strength <= 0) {
        ...
        this.starArray[i].strength = this.starArray[i].strength + random(-0.5, 0.5);
      } else {
        this.starArray[i].strength = this.starArray[i].strength - 0.05;
      }
    }
    show() {
      for (i = 0; i < this.starArray.length; i++) {
  ...
  class Wave {
    ...
    yaw() {
      this.finalRotation = random(-0.05, 0.05);
    ...
    edges() {
      ...
      if (this.location.y < this.originY - 20) {
  ...
  class Ship {
    ...
    edges() {
      if (this.location.x < this.originX - 400 || this.location.x > this.originX + 400) {
        this.velocity.x *= -1;
      }
      if (this.location.y < this.originY - 5 || this.location.y > this.originY + 20) {
        this.velocity.y *= -1;
      }
    }
    show() {
